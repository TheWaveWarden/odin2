...
#include "AudioParameterConnections.h" // constains the connection between raw float pointers and their ValueTree counter
  ...
  m_mod_matrix.setSourcesToRender = [&](bool p_LFO_0, bool p_LFO_1, bool p_LFO_2, bool p_LFO_3, bool p_ADSR_0, bool p_ADSR_1) {
                                      ...
                                      MidiBuffer &midiMessages)
{
  ...
  for (int sample = 0; sample < buffer.getNumSamples(); ++sample)
  {
    ...
    while (midi_message_remaining && midi_message_sample <= sample)
    {
      ...
      else if (midi_message.isController() && midi_message.getControllerNumber() == 1)
      { //modwheel
