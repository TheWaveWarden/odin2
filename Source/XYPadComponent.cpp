...
void XYPadComponent::paint(Graphics &g) {
  ...
  float x_handle = HANDLE_INLAY + m_value_x * (getWidth() - HANDLE_DIAMETER -
                                               2 * HANDLE_INLAY);
  float y_handle = getHeight() - HANDLE_DIAMETER -
                   (HANDLE_INLAY + m_value_y * (getHeight() - HANDLE_DIAMETER -
                                                2 * HANDLE_INLAY));
  if (!m_vector_pad) {
    g.setColour(Colour(0, 10, 30));
    g.drawLine(m_inlay, y_handle + HANDLE_DIAMETER / 2, getWidth() - m_inlay,
               y_handle + HANDLE_DIAMETER / 2);
    g.drawLine(x_handle + HANDLE_DIAMETER / 2, m_inlay,
               x_handle + HANDLE_DIAMETER / 2, getHeight() - m_inlay);
  }
...
void XYPadComponent::mouseDrag(const MouseEvent &event) { mouseInteraction(); }

void XYPadComponent::mouseDown(const MouseEvent &event) { mouseInteraction(); }
...
void XYPadComponent::mouseInteraction() {
  ...
  m_value_y = (float)(mouse_pos.getY() + HANDLE_DIAMETER / 2 - getHeight() +
                      HANDLE_INLAY) /
