...
void AnalogOscillator::loadWavetables() {
  ...
  setWavetablePointer(
      3, WavetableContainer::getInstance().getWavetablePointers("Sine"));
...
float AnalogOscillator::doSquare() {
  ...
  int read_index_next_offset = read_index_trunc_offset + 1 >= WAVETABLE_LENGTH
                                   ? 0
                                   : read_index_trunc_offset + 1;
  ...
  float output_offset = linearInterpolation(
      m_current_table[read_index_trunc_offset],
      m_current_table[read_index_next_offset], fractional_offset);
