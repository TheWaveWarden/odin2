...
int WavetableOsc1D::getTableIndex() {
  ...
  if(m_one_over_last_mid_freq * m_osc_freq_modded < ROOT_OF_MINOR_THIRD && m_one_over_last_mid_freq * m_osc_freq_modded > ONE_OVER_ROOT_OF_MINOR_THIRD){
  ...
  for (int table = 0; table < SUBTABLES_PER_WAVETABLE; ++table) {
    if (abs_freq < seed_freq) {
      //todo check if this works as intended... profiling showed some janky results in vecor osc?
      //todo same for chiptune oscillator... maybe logic not as sound as intended?
...
void WavetableOsc1D::setWavetablePointer(
  ...
  for (int sub_table = 0; sub_table < SUBTABLES_PER_WAVETABLE; sub_table++) {
    m_wavetable_pointers[p_wavetable_index][sub_table] =
...
void WavetableOsc1D::loadWavetables() {
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    setWavetablePointerNONCONST(
        NUMBER_OF_WAVETABLES + i,
        WavetableContainer::getInstance().getWavedrawPointer(i));
    // chipdraw
    setWavetablePointerNONCONST(
        NUMBER_OF_WAVETABLES + i + 3,
        WavetableContainer::getInstance().getChipdrawPointer(i));
    // specdraw
    setWavetablePointerNONCONST(
        NUMBER_OF_WAVETABLES + i + 6,
...
void WavetableOsc1D::loadWavedrawTables(int p_osc) {
  setWavetablePointerNONCONST(
      0, WavetableContainer::getInstance().getWavedrawPointer(p_osc));
  m_nr_of_wavetables = 1;
}

void WavetableOsc1D::loadSpecdrawTables(int p_osc) {
  setWavetablePointerNONCONST(
      0, WavetableContainer::getInstance().getSpecdrawPointer(p_osc));
  m_nr_of_wavetables = 1;
}

void WavetableOsc1D::loadChipdrawTables(int p_osc) {
  setWavetablePointerNONCONST(
      0, WavetableContainer::getInstance().getChipdrawPointer(p_osc));
  m_nr_of_wavetables = 1;
}
