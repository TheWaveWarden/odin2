...
void ADSR::testADSRPow(){
    ...
    for(size_t i = 0; i <  10000; ++i){
        output << calcDecayFactor(((double)i+0.5)*0.001) - calcDecayFactorCheap(((double)i+0.5)*0.001) << "," << calcDecayFactor(((double)i+0.5)*0.001) << "," << calcDecayFactorCheap(((double)i+0.5)*0.001) << "," << ((double)i+0.5)*0.001 << "\n";
    }
    output.close();
}
