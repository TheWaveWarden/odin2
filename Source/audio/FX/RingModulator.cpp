...
float RingModulator::doRingModulator(float p_input) {
  ...
  return p_input * (doOscillate() * amount_modded + (1.f - amount_modded)) *
         vol_mod_factor;
}
...
void RingModulator::update() {
  ...
  float kbd_modded = m_kbd_mod_amount + *m_kbd_mod_mod < 0
                         ? 0
  ...
  m_osc_freq_modded =
      m_osc_freq_base *
      pitchShiftMultiplier((*m_pitch_mod_exp) * OSC_EXP_MOD_RANGE +
