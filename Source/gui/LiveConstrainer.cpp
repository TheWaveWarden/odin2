...
LiveConstrainer::LiveConstrainer (SplineEditor& p_editor, JsonGuiProvider& p_json_gui) :
    m_main_editor (p_editor), m_gui_data (p_json_gui) {
	...
	if (p_event.eventComponent != m_component) {
		...
		if (p_event.mods.isShiftDown() && p_event.mods.isCtrlDown() && p_event.mods.isAltDown()) {
			...
			if (m_component->getProperties().contains (GUI_DATA_NAME)) {
				...
				m_bounds_scaled = m_gui_data.getBounds (m_name);
				m_bounds        = m_gui_data.getBoundsUnscaled (m_name);
...
void LiveConstrainer::mouseDrag (const juce::MouseEvent& p_event) {
	...
	const auto scale = int (SplineConfigFileManager::getInstance().getOptionGuiScale()) * 2.0f;
...
void LiveConstrainer::updateComponent() {
	if (m_component && m_component->getParentComponent()) {
		m_gui_data.setBoundsUnscaled (m_name, m_new_bounds);
...
void LiveConstrainer::paintOverlay (juce::Graphics& g) {
	...
	if (m_component) {
		...
		auto* editor = m_component->findParentComponentOfClass<SplineEditor>();
		...
		auto textBounds = editor->getLocalBounds().removeFromRight (editor->proportionOfWidth (0.2f)).removeFromTop (editor->proportionOfHeight (0.2f));
		...
		auto new_bounds_scaled = m_gui_data.getBounds (m_name);
		auto zoom_size         = int (m_main_editor.getZoomSize());
		...
		g.drawText ("Zoom: " + juce::String (juce::roundToInt(100.0f / 6.0f * float(zoom_size))) + "% (x" + juce::String (zoom_size) + ")", bounds, juce::Justification::centredRight);
		bounds = textBounds.removeFromTop (line_height);
		...
		g.drawLine (outline_bounds.getX(), outline_bounds.getCentreY(), outline_bounds.getRight(), outline_bounds.getCentreY(), stroke_size);
		g.drawLine (outline_bounds.getCentreX(), outline_bounds.getY(), outline_bounds.getCentreX(), outline_bounds.getBottom(), stroke_size);
	}
}
...
juce::MouseCursor LiveConstrainer::positionToMouseCursor (juce::Point<float> p_pos) {
	...
	const auto scale = int (SplineConfigFileManager::getInstance().getOptionGuiScale());
