...
void RescaleProgressComponent::paint(juce::Graphics &g) {
	auto colour_background = juce::Colours::slategrey;
	g.setGradientFill(
	    ColourGradient(colour_background.darker(0.4f), 0, H, colour_background.brighter(0.04f), W, 0, false));
	...
	auto progress_bounds =
	    progress_shell.withRight(progress_shell.getX() + progress_shell.proportionOfWidth(m_progress));
	...
	g.drawText("Rescaling UI Assets... ",
	           progress_shell.translated(0, -progress_shell.proportionOfHeight(1.5f)),
	           juce::Justification::centred,
	           true);
	g.drawText(juce::String(m_progress * 100.0f, 1) + "%",
	           progress_shell.translated(0, progress_shell.proportionOfHeight(1.5f)),
	           juce::Justification::centred,
	           true);
}