...
void GuiDataEditor::paint (juce::Graphics& g) {
	...
	g.drawText (juce::String (m_data.getWidth()) + "x" + juce::String (m_data.getHeight()), textBounds.removeFromTop (line_height), just);
}

void GuiDataEditor::setTopBottom (const juce::Rectangle<int>& p_top, const juce::Rectangle<int>& p_bottom) {
	m_top    = juce::Rectangle<int> (p_top.getX() * 2, p_top.getY() * 2, p_top.getWidth() * 2, p_top.getHeight() * 2);
	m_bottom = juce::Rectangle<int> (p_bottom.getX() * 2, p_bottom.getY() * 2, p_bottom.getWidth() * 2, p_bottom.getHeight() * 2);
}
