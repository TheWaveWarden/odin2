...
//==============================================================================
FilterComponent::FilterComponent() : m_vowel_left(false), m_vowel_right(true) {
  juce::Image metal_knob_big = ImageCache::getFromFile(juce::File(
      GRAPHICS_PATH + "cropped/knobs/metal3/metal_knob_big.png"));
  juce::Image metal_knob_mid = ImageCache::getFromFile(juce::File(
      GRAPHICS_PATH + "cropped/knobs/metal2/metal_knob_mid.png"));
  juce::Image black_knob_small = ImageCache::getFromFile(juce::File(
  ...
  m_vel.setKnobTooltip("Velocity sensitivity. Sets how much the MIDI velocity value is added to the filter freqency");
  ...
  m_freq.setSize(metal_knob_big.getWidth(), metal_knob_big.getHeight()/N_KNOB_FRAMES);
  ...
  m_sem_transition.setSize(metal_knob_mid.getWidth(), metal_knob_mid.getHeight()/N_KNOB_FRAMES);
  ...
  m_sem_transition.setDoubleClickReturnValue(true, TRANSITION_DEFAULT, ModifierKeys::ctrlModifier);
  m_sem_transition.setNumDecimalPlacesToDisplay(3);
  m_sem_transition.setKnobTooltip("Sets the filter characteristic\nfrom a lowpass over a\nnotch to a highpass");
  ...
  m_formant_transition.setSize(metal_knob_big.getWidth(), metal_knob_big.getHeight()/N_KNOB_FRAMES);
